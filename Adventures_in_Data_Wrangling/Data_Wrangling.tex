...
\begin{frame}
    ...
    \begin{itemize}
        \item Not cloud-based SaaS — already well studied, especially for corporates.
        
        \item Typical licensing models for small developers:
        \begin{itemize}
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item separate licenses for each platform {\tiny (to use \emph{all} features)}
            \item syncing must be manually configured
            \item major version upgrades are paid {\tiny (e.g., v6\(\rightarrow\)v7 \todo{USD\,39.99?})}
        \end{itemize}
        ...
        \begin{itemize}
            ...
            \item cost USD\,2.99/month (individual), USD\,4.99/month (family)\tinyskip
            {\tiny (when billed annually)}
        \end{itemize}
    \end{itemize}
    
    \begin{tikzpicture}[remember picture, overlay]
        \node[below left=1.5cm of current page.north east)] (icon) {
...
\begin{frame}
    ...
    \begin{tabular}{rrr}
        \toprule
        \textbf{Version}    &   \textbf{Released}   &   \textbf{Cost (USD)} \\
        \midrule
        2/3                 &   2009                &   39.95   \\
        5                   &   2014                &   24.99   \\
        6                   &   \todo{20??}         &   \todo{??}   \\
    ...
    \begin{itemize}
        \item Cost comparision:
        \begin{itemize}
            \item individual subscription 2009–2018 = 10 × USD\,2.99 × 12 = USD\,358.80 = USD\,35.88/year
        \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}
    ...
    \structurebf{To compare costs we need historical pricing data}
    \medskip
    
    \begin{itemize}
        \item No-one’s likely to publish their pricing history. {\tiny (who cares?)}
        \begin{itemize}
            \item except vendors that sell separate versions for different OS releases {\tiny (e.g. macOS Mail plugins)}
            \item at best probably only full pricing available, not upgrade
        \end{itemize}
        
        \item Wayback Machine? {\tiny (Internet Archive)}
        \begin{itemize}
            \item have been able to find a couple of old store-fronts
