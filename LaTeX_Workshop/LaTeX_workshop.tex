...
\newcommand{\instruction}[1]{\bigskip\noindent\fbox{\parbox{\columnwidth}{#1}}\bigskip}
...
\instruction{Download the file \textsf{small2e.tex} from the ISPG web
site and open it with WinShell.}
...
article). The document class can be thought of as a document template
that sets up a whole collection of defaults and standard formats. For
...
commands for specifying things like the address, the signature, and so on.
The standard document classes are \textsf{article}, \textsf{book},
...
specify. For example, document classes often default to a 10 point font.
You can change this to 12 point by saying
...
command is \verb+documentclass+, and its argument (enclosed by braces) is
\verb+article+. Looking further through the document, you'll see several
more examples of commands, such as \verb+\emph+ and \verb+\textbf+. Most
commands work similarly to these. Line 15 has an example of a sectioning
command: \verb+\section+. This defines a logical section of the document.
A subsection is defined on line 29.
...
\begin{itemize}
	...
	\item \verb+~+ alone is a non-breaking space.
	
	\item The special command \verb+\\+ inserts a line break (make sure
	you put a space after it if you use it in the middle of a line).
	
	\item To produce the three remaining special characters, use
	\verb+$\backslash$+ for $\backslash$, \verb+\^{}+ for \^{} and
	\verb+\~{}+ for \~{}.
	
	\item You can italicise text using \verb+\textit+. However, you're
	generally better to use \verb+\emph+ to emphasise text. The nice
	thing about \verb+\emph+ is that you can nest it, and the italics
	will turn on and off as required: \emph{Use italics to emphasise
	text, \emph{unless} you're already in italics}.
	
	\item \verb+\textsf+ produces \textsf{sans-serif text}, while
	\verb+\texttt+ produces \texttt{typewriter text}.
	...
	careful using commands like this in the middle of a sentence, however,
	because they ``eat'' any spaces that occur after them, e.g.,
	\verb+\today is the day+ produces ``\today is the day''. This is
	just a peculiarity of the way the engine works. You can fix this by
	putting a backslash at the end of the command, i.e.,
	\verb+\today\ is the day+.
	
	\item Command names are case-sensitive, so \verb+\Today+ is
	considered a different command from \verb+\today+. Command names can
	also only contain letters; no numbers or unusual characters.
...
\instruction{Download the file \textsf{sample2e.tex} from the ISPG web
site, open it in WinShell and process it with \PDFLaTeX.}

This file is a somewhat longer document that includes most of the
features discussed above, plus a few extras. It's a good source of
...
commands won't work across paragraphs, so what do we do if we want to
...
commands (e.g., a \textsf{em} environment that does the same as
...
or numbers aligned in columns (like Table~\ref{tab:lists} above). This
...
\end{tabular}
\end{verbatim}
This comes out looking like this:\bigskip
...
that you can reference them. \LaTeX\ provides two environments for doing
this: \textsf{figure} and \textsf{table}. \textsf{Figure} is generally
used for graphical material and \textsf{table} for tabular material, but
that's just by convention---\LaTeX\ doesn't really care.
...
and Table~\ref{tab:lists}. Table~\ref{tab:lists} is a good example of
the floating aspect: it has floated to the top of
page~\pageref{tab:lists} and the main text has simply flowed itself
around the table. We have some control over this (and occasionally
\LaTeX\ does get it wrong), but it's usually safer to leave \LaTeX\
alone.
...
that are automatically kept up to date. You create labels by placing a
\verb+\label+ command immediately after the item that you want to label.
For example, the heading of this section looks like this:
\begin{verbatim}
\subsubsection{Cross-references}
\label{sec:xref}
\end{verbatim}
...
Once a label is defined, you can cross-reference the item using
\verb+\ref+, e.g., \verb+\ref{sec:xref}+ will generate the number
for this section (\ref{sec:xref}). If you change the position of this
section in the document, the section number and all references to it
will update automatically the next time you process the document. You
...
warn you about any references it doesn't recognise.
...
formatting of references. Using \BibTeX\ involves several steps,
...
\verb+\cite{UsersGuide}+ will produce the citation
``\cite{UsersGuide}''.
...
defines a new set of commands. There are hundreds, if not thousands, of
...
Archive Network (CTAN)~\cite{CTAN}, which is a worldwide network
of mirrored FTP sites for \TeX\ and \LaTeX. If you want something, it's
...
\begin{description}
	...
	default margins are quite large---this has to do with various
...
\usepackage[a4paper,left=2cm,right=3cm,vmargin=2cm]{geometry}
	...
	\PDFLaTeX\ doesn't support EPS). The main command provided by this
	packages is \verb+\includegraphics+.
	...
	Times, the sans-serif font to Helvetica and the typewriter font to
	Courier. The only problem with doing this is that maths mode still
	uses the Computer Modern fonts, which can look a little odd. The
	\textsf{mathptm} (Times) and \textsf{mathpple} (Palatino) packages
	also change the fonts for maths mode, so if your document includes a
	lot of maths, you might consider using one of these.
	...
	makes it difficult to include them in documents. This package
	provides the \verb+\url+ command that lets you avoid these problems.
	(The \textsf{harvard} package also supports URLs in bibliographic
	references.)
...
\begin{itemize}
	...
	\item Buy yourself a copy of Lamport's book~\cite{UsersGuide}, and
	preferably also \emph{The \LaTeX\ Companion}~\cite{Companion}.
	
	\item You'll find documentation for most packages buried somewhere
	in the \texttt{texmf} documentation tree. If you need to do complex
	graphics, you'll want a copy of \emph{The \LaTeX\ Graphics
	Companion}~\cite{Graphics}.
...
% The LaTeX Companion
% LaTeX tutorials (search in Google)
