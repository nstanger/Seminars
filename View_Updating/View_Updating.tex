...
\frame
{
    ...
    \structurebf{\(\bm{P\{\mathit{Pno}, \mathit{Pname}, \mathit{Color}, \mathit{Weight}, \City\}}\)}
...
\frame
{
    ...
    \begin{itemize}
        ...
        \item \emph{All} constraints explicitly specified in advance.
        ...
        \begin{itemize}
            ...
            \hfill{\tiny Date (2013) \emph{View Updating and Relational Theory}}
...
{
    \frametitle{Example SIG for relvar \(\bm{S\{\Sno, \Sname, \Status, \City\}}\)}
    \framesubtitle{\hyperlink{frame.SIG.Sone}{\beamerreturnbutton{Back}}}
...
\begin{frame}[fragile]
    ...
    \begin{tabular}{lll}
        ...
        \midrule
        restrict (\(\RelRestrict\))  &   selection edge  &   \(A \SelectionEdge B\) (or \(A \TrivialSelection B\))\footnote{\label{triv}Trivial case.} \\
        project (\(\RelProject\))  &   projection edge  &   \(A \ProjectionEdge B\) (or \(A \TrivialProjection B\))\footnotemark[4] \\
        \bottomrule
    \end{tabular}
\end{frame}
...
\frame
{
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item transform \(\SC{1}\) to be isomorphic to \(\SC{2}\) via equivalence preserving transformations \(\Rightarrow\Equivalent{\SC{1}}{\SC{2}}\) {\tiny (or vice versa)}
            \item transform \(\SC{1}\) to be isomorphic to \(\SC{2}\) via information preserving transformations \(\Rightarrow\Dominates{\SC{2}}{\SC{1}}\) {\tiny (or vice versa)}
        ...
        \begin{itemize}
            ...
            \item add bijective (trivial) selection edge (\(\equiv\))
            \item delete bijective (trivial) selection edge (\(\preceq\))
        \end{itemize}
    ...
    \begin{columns}
        \begin{column}[T]{0.5\textwidth}
            ...
            \begin{itemize}
                ...
                \item Sub-schemas \(\SC{1} = \{S\}\), \(\SC{2} = \{\LS, \NLS\}\), \(\SC{3} = \{LS\}\)
                \item Date says \((\SC{1}, \SC{2})\) are informa- tion equivalent, \((\SC{1}, \SC{3})\) aren’t.\tinyskip
        ...
        \begin{column}[T]{0.5\textwidth}
            \begin{uncoverenv}<2->
                ...
                \begin{ConstraintList}
                    \item \(\{\Sno\} \rightarrow \{\Sname, \Status, \City\}\) holds in (a) \(S\); (b) \(\LS\); and (c) \(\NLS\) [(primary) key]
...
\frame
{
    ...
    \framesubtitle{(ignoring FKs for now — \(\bm{\text{C}_{2}}\) and \(\bm{\text{C}_{3}}\))}
...
\frame[label=frame.SIG.transform]
{
    ...
    \framesubtitle{\alt<1-8>{\textcolor{structure}{blue} = input to transformation, \textcolor{alert}{red} = output}{\hyperlink{frame.SIG.Sone<2>}{\beamergotobutton{Compare}}}}
