...
\frame
{
    ...
    \framesubtitle{(Keller (1985), \emph{Algorithms for translating view updates to database updates for views involving selections, projections, and joins}, PODS)}
...
\frame
{
    ...
    \begin{columns}
        \begin{column}[T]{0.45\textwidth}
            ...
            \begin{tabular}{|l|l|r|l|}
                ...
                S2                  & Jones                 & 30                    & Paris \\
                S3                  & Blake                 & 30                    & Paris \\
                \only<1-2,4-5>{S4}  & \only<1-2,4-5>{Clark} & \only<1-2,4-5>{20}    & \only<1-2,4-5>{\alt<1-4>{London}{\alert{Foo}}} \\
                S5                  & Adams                 & 30                    & Athens \\
                \hline
            \end{tabular}
        \end{column}
        \begin{column}[T]{0.45\textwidth}
            ...
            \begin{tabular}{|l|l|r|l|}
                ...
                \hline
                S1              & Smith             & 20                & London \\
                \only<1-2,4>{S4} & \only<1-2,4>{Clark}& \only<1-2,4>{20}   & \only<1-2,4>{London} \\
                \hline
            \end{tabular}
        \end{column}
    \end{columns}
    ...
    \begin{enumerate}
        ...
        \item<4-5> Change S4’s city to anything other than London.
    \end{enumerate}
}
...
%     \structurebf{What about \(\bm{\mathsf{SSP} = \mathsf{S} \RelNaturalJoin \mathsf{SP}}\)?}
    ...
    \begin{columns}
        ...
        \begin{column}[T]{0.5\textwidth}
            ...
            \footnotesize\structurebf{Delete first row of SSP?}
            
            \scriptsize
            \begin{enumerate}
                \item Delete S1.
                \item Delete P1.
                \item Delete \emph{both} S1 and P1
                \item Change S1’s supplier number.
                \item Change P1’s part number.
                \item Change \emph{both} S1’s supplier number and P1’s part number.
            \end{enumerate}
        \end{column}
    \end{columns}
}
