	...
	\begin{columns}
		\begin{column}{12cm}
			\begin{pgfpicture}{0cm}{0cm}{12cm}{6.25cm}
				\only<1>{\pgfputat{\pgfxy(6,3.125)}{\pgfbox[center,center]{\includegraphics[width=12cm,keepaspectratio]{XML-Workflow-Web}}}}
				\only<2>{\pgfputat{\pgfxy(6,3.125)}{\pgfbox[center,center]{\includegraphics[width=12cm,keepaspectratio]{XML-Workflow-TSV}}}}
				\only<3>{\pgfputat{\pgfxy(6,3.125)}{\pgfbox[center,center]{\includegraphics[width=12cm,keepaspectratio]{XML-Workflow-IS}}}}
			\end{pgfpicture}
		\end{column}
	\end{columns}
	
	\note{The XML will often be generated from the database with the aid
...
\frame
{
	...
	\begin{itemize}
		...
		\note[item]{Examples: stock quotes, search engines,
		authentication (Microsoft Passport, credit cards), The Captain
		Haddock Insult Generator, basically anything that can be
		encapsulated in a chunk of code and called remotely. Really just
		XML-enabled RPC over HTTP.}
		...
		\note[item]{Microsoft and Sun have clashed over standards for
